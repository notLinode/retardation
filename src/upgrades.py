...
_ALL_UPGRADES: list[_Upgrade] = [
    _Upgrade(
        u_id=0,
        name="🍗 ;feed",
        desc="Разблокирует команду `;feed`, с помощью которой можно кормить бота пользовательской едой.",
        cost=50,
    ...
    _Upgrade(
        u_id=1,
        name="🩷 ;heal",
        desc="Разблокирует команду `;heal`, с помощью которой можно лечить бота пользовательскими лекарствами.",
        cost=50,
        TYPE=_UpgradeType.GLOBAL
    ),
    _U_AfkTokens(
    ...
    _U_Fubar(
        u_id=3,
        name="👹 Ты ебанутый",
        desc="Добавляет \"ТЫ ЕБАНУТЫЙ\" к запросу сообщений инвалида на час.",
        cost=15,
        TYPE=_UpgradeType.GLOBAL
    )
    ]
